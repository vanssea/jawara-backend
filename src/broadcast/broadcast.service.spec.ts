...
describe('BroadcastService', () => {
  ...
  describe('create', () => {
    it('should insert broadcast and log activity then return data', async () => {
      const fakeData = { id: '1', judul: 'Halo', isi: 'Isi' };

      // first insert (broadcast) -> returns { data, error }
      ...
      const result = await service.create('user-1', { judul: 'Halo', isi: 'Isi' } as any);

      expect(result).toEqual(fakeData);
      expect(mockSupabaseService.getClient).toHaveBeenCalled();
      expect(spyCreateActivity).toHaveBeenCalledWith('broadcast', 'Halo');
      // ensure an insert happened (broadcast) and a second insert for logs
      expect(mockInsert).toHaveBeenCalled();
    });

    it('should throw HttpException when broadcast insert returns error', async () => {
      // broadcast insert returns error
      mockInsert.mockReturnValueOnce({
        select: () => ({ single: () => Promise.resolve({ data: null, error: { message: 'DB fail' } }) }),
      });

      await expect(service.create('u1', { judul: 'X', isi: 'Y' } as any)).rejects.toThrow(HttpException);
    });

    it('should throw HttpException when log insertion fails', async () => {
      const fakeData = { id: '1', judul: 'Halo' };

      // broadcast insert success
      mockInsert.mockReturnValueOnce({
        select: () => ({ single: () => Promise.resolve({ data: fakeData, error: null }) }),
      });
      ...
      await expect(service.create('u1', { judul: 'Halo', isi: 'Y' } as any)).rejects.toThrow(HttpException);
    });
  });
  ...
  describe('update', () => {
    it('should update broadcast and insert log then return updated data', async () => {
      const updated = { id: '1', judul: 'Updated' };
      ...
      const result = await service.update('u1', '1', { judul: 'Updated' } as any);

      expect(result).toEqual(updated);
      expect(spyUpdateActivity).toHaveBeenCalledWith('broadcast', updated.judul);
    });
    ...
    it('should throw HttpException when update returns error', async () => {
      ...
      await expect(service.update('u1', '1', { judul: 'X' } as any)).rejects.toThrow(HttpException);
    });
    ...
    it('should throw HttpException when log insert fails after update', async () => {
      ...
      await expect(service.update('u1', '1', { judul: 'Updated' } as any)).rejects.toThrow(HttpException);
    });
  });

  describe('remove', () => {
    it('should delete broadcast and insert log', async () => {
      const item = { id: '1', judul: 'ToDelete' };
      ...
      expect(spyDeleteActivity).toHaveBeenCalledWith('broadcast', item.judul);
    });

    it('should throw HttpException when delete returns error', async () => {
      jest.spyOn(service, 'findOne').mockResolvedValue({ id: '1', judul: 'X' } as any);
